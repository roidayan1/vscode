...
class SessionsTreeDelegate implements IListVirtualDelegate<SessionListItem> {
	...
	getHeight(element: SessionListItem, reader?: IReader): number {
		...
		if (this._approvalModel) {
			...
			const approval = getSessionRowApproval(this._approvalModel, element, reader, this._aggregateChatApprovals);
			if (approval) {
...
export class SessionsFlatList extends Disposable {
	...
	constructor(
		...
		const chatRenderer = showChatChildren ? new SessionChatItemRenderer(hoverService, instantiationService, markdownRendererService, approvalModel, approvalRowMaxLines, activeGuideSessionIds) : undefined;
		this._delegate = new SessionsTreeDelegate(approvalModel, () => false, approvalRowMaxLines, this.options.ciFixModel, useCompactQuickChatRows, !showChatChildren, showChatChildren);
